...
def debug_eager(iterable, label: str | None = None, max_count: int = 100):
    ...
    prints them, then passes them on. This is similar to {py:func}`tee`, but {py:func}`tee` is completely
    lazy, and will print items one-by-one, whereas this function will show the entire
    batch at once.
